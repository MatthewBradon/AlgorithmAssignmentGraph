...
class Heap {
    ...
    public void siftUp(int k) {
        ...
        System.out.println("SiftUp: Sifting " + v + " up");
        System.out.println("Before siftUp: ");
        this.print();
        while (dist[v] < dist[k / 2]) {
            a[k] = a[k / 2];
            hPos[k] = k;
            k = k / 2;
    ...
    public void siftDown(int k) {
        ...
        v = a[k];
        j = 2 * k;
        System.out.println("SiftDown Sifting " + v + " down");
        System.out.println("Before siftDown: ");
        this.print();
        while (j <= N - 1) {

            if (dist[a[j]] < dist[a[j + 1]] && j < N - 1) {
                j++;
            }
            if (dist[v] <= dist[a[j]]) {
                break;
    ...
    public void insert(int x) {
        System.out.println("Inserting " + x + " current heap:");
        this.print();
        a[++N] = x;
        siftUp(N);
    }
    ...
    public int remove() {
        System.out.println("Removing " + a[1] + " current heap:");
        this.print();
        int v = a[1];
...
class Graph {
    ...
    public Graph(String graphFile) throws IOException {
        ...
        for (e = 1; e <= E; ++e) {
            ...
            t = adj[u];
            if (t == z) {
                adj[u] = new Node(v, wgt, z);
            } else {
                while (t.next != z) {
                    t = t.next;
                }
                t.next = new Node(v, wgt, z);
            }

            t = adj[v];
            if (t == z) {
                adj[v] = new Node(u, wgt, z);
            } else {
                while (t.next != z) {
                    t = t.next;
                }
                t.next = new Node(u, wgt, z);
            }
